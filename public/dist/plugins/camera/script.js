$(document).ready(function(){
	...
	var camera = $('#camera'),
		photos = $('#photos'),
		screen =  $('#screen');
		url = '/shinedev/content/uploads/profiles/';
	...
	webcam.set_swf_url('/shinedev/content/includes/js/camera/webcam.swf');
	//webcam.set_api_url('/shinedev/file/do_upload/profiles');	// The upload script
	webcam.set_quality(90);								// JPEG Photo Quality
	webcam.set_shutter_sound(true, '/shinedev/content/includes/js/camera/shutter.mp3');
	...
	webcam.set_hook('onComplete', function(msg){
		...
		if(msg.error){
			...
			$('#memimg').css('background-image','url("'+url+msg.filename+'")' );
			$('div.profile_image').css('background-image','url("'+url+msg.filename+'")' );
		}
	});
