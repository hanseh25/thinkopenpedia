...
/*
 * This file is part of ShineOS+
 *
 * (c) ShineOS+
 *
 *
 */
namespace ShineOS\Providers;
...
class UrlManager {
    ...
    function link_to_file($path) {
        ...
        $path = str_ireplace(SHINEOS_ROOTPATH, '', $path);
    ...
    function to_path($path) {
        ...
        $path = str_ireplace($this->site_url(), SHINEOS_ROOTPATH, $path);
        $path = str_replace('\\', '/', $path);
        $path = str_replace('//', '/', $path);
    ...
    public function params($skip_ajax = false) {
        return $this->param($param = "__SHINEOS_GET_ALL_PARAMS__", $skip_ajax);
    }
    ...
    public function param($param, $skip_ajax = false, $force_url = false) {
        ...
        foreach ($segs as $segment) {
            ...
            if ($param=='__SHINEOS_GET_ALL_PARAMS__'){
    ...
    public function download($requestUrl, $post_params = false, $save_to_file = false) {
        ...
        $opts = array('http' => array('method' => 'POST', 'header' => "User-Agent: Microweber/" . SHINEOS_VERSION . "\r\n" . 'Content-type: application/x-www-form-urlencoded' . "\r\n" . 'Referer: ' . $ref . "\r\n", 'content' => $postdata));
        $requestUrl = str_replace(' ', '%20', $requestUrl);

        if (function_exists('curl_init')){
            $ch = curl_init($requestUrl);
            curl_setopt($ch, CURLOPT_COOKIEJAR, shineos_cache_path() . "global/cookie.txt");
            curl_setopt($ch, CURLOPT_COOKIEFILE, shineos_cache_path() . "global/cookie.txt");
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
            curl_setopt($ch, CURLOPT_USERAGENT, "Mozilla/4.0 (compatible; MSIE 5.01; Microweber " . SHINEOS_VERSION . ";)");
