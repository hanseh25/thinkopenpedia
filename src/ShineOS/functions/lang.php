...
$shineos_language_content_saved = false;
$shineos_new_language_entires = array();
$shineos_new_language_entires_ns = array();
...
{
    global $shineos_new_language_entires_ns;
    global $shineos_new_language_entires_ns;
    $lang = current_lang();
    if (!empty($shineos_new_language_entires_ns)) {
        foreach ($shineos_new_language_entires_ns as $k => $v) {
            $namespace = $k;
            ...
            $existing = $shineos_new_language_entires_ns[$k];
            if (is_array($v)) {
...
function __store_lang_file()
{
    ...
    global $shineos_language_content_saved;


    if ($shineos_language_content_saved == true) {
        return;
    }
    if (is_admin() == false) {
        return false;
    }
    global $shineos_new_language_entires;
    $shineos_language_content = get_language_file_content();

    $lang = current_lang();


    $lang_file = userfiles_path() . 'language' . DIRECTORY_SEPARATOR . $lang . '.json';
    if (!is_array($shineos_language_content) or empty($shineos_language_content)) {

        $lang_file_str = json_encode($shineos_new_language_entires, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES);

        @file_put_contents($lang_file, $lang_file_str);

    }
    $shineos_language_content2 = array();
    if (is_array($shineos_language_content) and is_array($shineos_new_language_entires) and !empty($shineos_new_language_entires)) {

        $shineos_language_content2 = $shineos_new_language_entires;

        if (!empty($shineos_language_content2)) {
            foreach ($shineos_language_content2 as $key => $value) {

                if (!isset($shineos_language_content[$key])) {

                    $shineos_language_content[$key] = $value;
                }
            }
        ...
        $shineos_language_content = array_unique($shineos_language_content);


        $lang_file_str = json_encode($shineos_language_content, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES);
        $shineos_language_content_saved = 1;
        if (is_admin() == true) {
            $c1 = count($shineos_language_content);
            $c2 = count($shineos_language_content2);
...
function lang($title, $namespace = false)
{
    ...
    static $lang_file;
    global $shineos_language_content;
    global $shineos_new_language_entires_ns;
    $k = $title;
    //$k1 = strip_tags($k);
    $k1 = 'key'.crc32($k);
 ...
 //   $k1 = shineos()->url_manager->slug($k);

    $lang = current_lang();

    $shineos_language_content_file = get_language_file_content($namespace);

    if (isset($shineos_language_content_file[$k1]) == false) {
        if (is_admin() == true) {
            $namespace = trim($namespace);
            $namespace = str_replace(' ', '', $namespace);
            $namespace = str_replace('..', '', $namespace);
            $namespace = str_replace('\\', '/', $namespace);
            if (!isset($shineos_new_language_entires_ns[$namespace])) {
                $shineos_new_language_entires_ns[$namespace] = array();
            }
            $k2 = ($k);
            $shineos_new_language_entires_ns[$namespace][$k1] = $k2;

            if (!defined('MW_LANG_STORE_ON_EXIT_EVENT_BINDED_NS')) {
                define('MW_LANG_STORE_ON_EXIT_EVENT_BINDED_NS', 1);
                $scheduler = new \ShineOS\Providers\Event();
                $scheduler->registerShutdownEvent("__store_lang_file_ns");
            }
        }
        return $k;
    } else {
        return $shineos_language_content_file[$k1];
...
function _e($k, $to_return = false)
{
    ...
    if (isset($shineos_language_content[$k1]) == false) {
        //if (is_admin() == true) {
        $k2 = ($k);
        $shineos_new_language_entires[$k1] = $k2;
        $shineos_language_content[$k1] = $k2;
        ...
        if ($to_return == true) {
            return $shineos_language_content[$k1];
        }
        print $shineos_language_content[$k1];
    }
...
function send_lang_form_to_microweber($data)
{
    if (is_admin() == true) {
        ...
        $send['language'] = $lang;
        $send['data'] = $data;
        return shineos_send_anonymous_server_data($send);
...
function save_language_file_content($data)
{
    ...
    if (is_admin() == true) {
        ...
        // $cust_dir = $lang_file = shineos_includes_path() . 'language' . DIRECTORY_SEPARATOR . 'custom' . DIRECTORY_SEPARATOR;
        ...
        $shineos_language_content = $data;

        $lang_file = $cust_dir . $lang . '.json';

        if (is_array($shineos_language_content)) {
            $shineos_language_content = array_unique($shineos_language_content);
            $lang_file_str = json_encode($shineos_language_content, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES);
...
$shineos_language_content = array();
$shineos_language_content_namespace = array();
...
{
    if ($namespace == false) {
        return _shineos_get_language_file_content_core();
    } elseif ($namespace != false) {
        return _shineos_get_language_file_content_namespaced($namespace);
    }
}

function _shineos_get_language_file_content_core()
{
    global $shineos_language_content;

    if (!empty($shineos_language_content)) {
        return $shineos_language_content;
    }

    $lang = current_lang();
    $lang_file = shineos_includes_path() . 'language' . DIRECTORY_SEPARATOR . $lang . '.json';
    $lang_file = normalize_path($lang_file, false);
    $lang_file2 = userfiles_path() . 'language' . DIRECTORY_SEPARATOR . $lang . '.json';
    $lang_file3 = shineos_includes_path() . 'language' . DIRECTORY_SEPARATOR . 'en.json';
    ...
    if (is_file($lang_file2)) {
        ...
        if (isset($language) and is_array($language)) {
            foreach ($language as $k => $v) {
                if (isset($shineos_language_content[$k]) == false) {
                    $shineos_language_content[$k] = $v;
                }
            }
        }
    }
...
function _shineos_get_language_file_content_namespaced($namespace)
{
    if ($namespace == false) {
        return false;
    }

    global $shineos_language_content_namespace;
    $namespace = trim($namespace);
    $namespace = str_replace(' ', '', $namespace);
    $namespace = str_replace('..', '', $namespace);
    $namespace = str_replace('\\', '/', $namespace);
    if (isset($shineos_language_content_namespace[$namespace]) and !empty($shineos_language_content_namespace[$namespace])) {
        return $shineos_language_content_namespace[$namespace];
    }
    ...
    if (!isset($shineos_language_content_namespace[$namespace])) {
        $shineos_language_content_namespace[$namespace] = array();
    }
    if (is_file($lang_file2)) {
        $language_str = file_get_contents($lang_file2);
        $language = json_decode($language_str, true);
        if (isset($language) and is_array($language)) {
            foreach ($language as $k => $v) {
                if (isset($shineos_language_content[$namespace][$k]) == false) {
                    $shineos_language_content_namespace[$namespace][$k] = $v;
                }
            }
        }
    }
    ...
    if (is_file($lang_file)) {
        ...
        if (isset($language) and is_array($language)) {
            foreach ($language as $k => $v) {
                if (isset($shineos_language_content_namespace[$namespace][$k]) == false) {
                    $shineos_language_content_namespace[$namespace][$k] = $v;
                }
            }
        }
...
{
    global $shineos_all_langs;

    if (!empty($shineos_all_langs)) {
        return $shineos_all_langs;
    }

    $lang_dir = shineos_includes_path() . 'language' . DIRECTORY_SEPARATOR;

    $files = array();
    foreach (glob($lang_dir . "*.json") as $filename) {
        $item = basename($filename);
        $item = no_ext($item);
        $shineos_all_langs[] = $item;
    }
...
function show_help($section = 'main')
{
    ...
    $lang_file = shineos_includes_path() . 'help' . DIRECTORY_SEPARATOR . $lang . '.php';
    $lang_file_en = shineos_includes_path() . 'help' . DIRECTORY_SEPARATOR . $lang . '.php';
    $lang_file = normalize_path($lang_file, false);
