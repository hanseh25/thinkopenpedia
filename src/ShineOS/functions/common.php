 ...
 *
 * @param null $class
 * @return \ShineOS\Application ShineOS Application object
 */
function shineos($class = null)
{

    return app($class);

    $app = \ShineOS\Application::getInstance();
...
if (!function_exists('site_url')) {
    function site_url($add_string = false)
    {
        ...
        if (defined('ShineOS_SITE_URL')) {
            $site_url = ShineOS_SITE_URL;
        }
        if ($site_url == false) {
            $pageURL = 'http';
...
{
    return shineos()->format->auto_link($text);
}

function prep_url($text)
{
    return shineos()->format->prep_url($text);
}
...
{
    return shineos()->url_manager->is_ajax();

}

function url_current($skip_ajax = false, $no_get = false)
{
    return shineos()->url_manager->current($skip_ajax, $no_get);
}

function url_segment($k = -1, $page_url = false)
{
    return shineos()->url_manager->segment($k, $page_url);
...
{
    return shineos()->url_manager->string($skip_ajax);
}
...
{
    return shineos()->url_manager->slug($text);
}

function url_param($param, $skip_ajax = false)
{
    return shineos()->url_manager->param($param, $skip_ajax);
}
...
{
    return shineos()->cache_manager->get($cache_id, $cache_group, $expiration_in_seconds);
}
...
{
    return shineos()->cache_manager->save($data_to_cache, $cache_id, $cache_group,$expiration);
...
function clearcache()
{
    return shineos()->cache_manager->clear();
...
{
    return shineos()->cache_manager->debug();
...
{
    return shineos()->cache_manager->delete($cache_group, $cache_storage_type);
}
