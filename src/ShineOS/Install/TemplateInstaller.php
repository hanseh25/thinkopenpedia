...
class TemplateInstaller
{
    ...
    public function run()
    {
        $selected_template = Config::get('shineos.install_default_template');
        $default_content = Config::get('shineos.install_default_template_content');
    ...
    {
        $default_content_folder = shineos_includes_path() . 'install' . DIRECTORY_SEPARATOR;
        $default_content_file = $default_content_folder . 'shineos_default_content.zip';
        ...
        if (($template_name)) {
            if (function_exists('templates_path')) {
                ...
                if (is_dir($template_dir)) {
                    $template_default_content = $template_dir . 'shineos_default_content.zip';
        ...
        if (is_file($default_content_file)) {
            $restore = new \ShineOS\Utils\Backup();
            $restore->backups_folder = $default_content_folder;
            $restore->backup_file = 'shineos_default_content.zip';
            ob_start();
            try {
